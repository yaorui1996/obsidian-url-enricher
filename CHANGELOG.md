...
### Added
- **Fork: Attachment URL exclusion.** URLs whose last path segment looks like a filename — has an extension that isn't a web-page suffix (`.html`/`.php`/…) — are now skipped entirely: no preview pill, no request, Obsidian's native link handling left untouched. Detection is static (URL text only, no network call) and applies to all three preview styles in Live Preview and Reading view. A new "Attachment handling" setting accepts comma/newline-separated substring rules (e.g. `/d/picgo/`) to also skip any matching URL. The fetch-title CLI flags attachments with `isAttachment: true`
